...
app.get("/api/movies/:id", async (req, res) => {
  ...
  try {
    ...
    console.error(
      `Could not retrieve movie with id ${req.params.id} from db`,
      error
    );
    res.send({});
  }
...
app.get("/api/movies", async (req, res) => {
  ...
  try {
    ...
    const result = await client.query(
      `SELECT id, movie_title, imdb_score FROM MOVIES order by imdb_score desc LIMIT 25 OFFSET ${req.query.offset}`
    );
